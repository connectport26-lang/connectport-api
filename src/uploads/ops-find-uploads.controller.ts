...
@Controller('ops/find-uploads')
@RequireOps()
@RequireOpsPermission('finds.submit')
export class OpsFindUploadsController {
  constructor(
  ...
  async upload(
    ...
    const key = `cp:ops-find-uploads:daily:${user.sub}:${day}`;
    ...
    try {
      const result = await this.uploads.storeFindMedia(file);
